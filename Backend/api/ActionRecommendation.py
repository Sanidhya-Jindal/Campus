...
class GeminiAlertRecommender:
    ...
    def __init__(self, model: str = None, prompt_template: str = None):
        """
        ...
        Args:
            model: Model to use (defaults to $GEMINI_MODEL, else gemini-flash-latest)
        ...
        # Initialize LangChain Gemini LLM. max_retries=0 and a request timeout
        # stop a rate-limited free-tier key (20 req/day) from turning the
        # cache-populating alerts request into a multi-minute retry-backoff hang;
        # on failure the per-alert static recommendations are kept instead.
...
Provide ONE clear, actionable and impactful in real world scenario recommendation following these guidelines."""
    ...
    def generate_recommendations(self, alerts_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # One LLM call per alert type, run concurrently — they are independent,
        # so 4 sequential ~10s calls become one ~10s wall-clock wait.
        recommendations_by_type = {}
        from concurrent.futures import ThreadPoolExecutor

        with ThreadPoolExecutor(max_workers=len(alerts_by_type) or 1) as pool:
            futures = {
                pool.submit(self._get_llm_recommendation, alert_type, alerts): alert_type
                for alert_type, alerts in alerts_by_type.items()
            }
            for future, alert_type in futures.items():
                recommendations_by_type[alert_type] = future.result()
