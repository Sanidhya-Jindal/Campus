...
AUTH_TOKEN = None
# Access tokens expire after 15 min; the agent logs in once at startup, so
# without these the chatbot dies 15 minutes in. Stored so _api_request() can
# silently re-authenticate on a 401.
_CREDENTIALS = {"email": None, "password": None}
...
def _api_request(method: str, url: str, **kwargs) -> requests.Response:
    """
    Makes an authenticated request, transparently re-logging-in and retrying
    once if the access token has expired (401).
    """
    response = requests.request(method, url, headers=_get_auth_headers(), **kwargs)
    if response.status_code == 401 and _CREDENTIALS["email"]:
        # Token expired mid-session — re-authenticate with stored credentials.
        result = login(_CREDENTIALS["email"], _CREDENTIALS["password"])
...
def search_entity(query: str) -> dict:
    ...
    try:
        response = _api_request(
            "get", f"{API_BASE_URL}/api/entities/", params={"q": query}
        )
        response.raise_for_status()
...
def predict_location(entity_id: str) -> dict:
    ...
    try:
        response = _api_request(
            "post", f"{API_BASE_URL}/api/predict/", json={"entity_id": entity_id}
        )
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
...
def get_alerts() -> dict:
    ...
    """
    try:
        response = _api_request("get", f"{API_BASE_URL}/api/alerts/")
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        error_detail = e.response.json().get('detail', str(e)) if e.response else str(e)
