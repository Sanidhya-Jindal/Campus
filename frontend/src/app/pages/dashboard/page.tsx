...
import { API_BASE, FACE_BASE } from "@/app/lib/api";
...
function Dashboard() {
  ...
  const fetchEntities = useCallback(async (q: string = "") => {
    ...
    try {
      const url = q
        ? `${API_BASE}/api/entities/?q=${encodeURIComponent(q)}`
  ...
  const fetchFace = async () => {
    ...
    try {
      const res = await fetch(`${FACE_BASE}/identify-and-search/`, {
        method: "POST",
        body: formData,
      });
      if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      ...
      <main className="mx-auto px-4 sm:px-6 lg:px-8 py-5">
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(280px,330px)_1fr] gap-4">
          {/* Left rail: at-a-glance counters + face lookup */}
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <StatTile
                label="Active Alerts"
                value={alertsCount === 0 ? "—" : String(alertsCount)}
                icon={<AlertTriangle className="w-4 h-4 text-red-400" />}
                tone="bg-red-500/10"
                hint={alertsCount === 0 ? "loading…" : undefined}
              />
              <StatTile
                label="Face Enrolled"
                value="5000"
              ...
              <StatTile
                label="System Status"
                value="Operational"
                valueClass="text-green-400 text-base"
            ...
            <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-800/50 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-sm font-semibold">Face Identification</h2>
              </div>
              <div className="flex flex-col gap-2">
                ...
                <label
                  htmlFor="face-upload"
                  className="flex items-center justify-center gap-2 w-full px-3 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-lg cursor-pointer hover:bg-slate-800 transition-colors"
                >
                ...
                <button
                  ...
                  className="px-4 py-2.5 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 disabled:from-slate-700 disabled:to-slate-700 disabled:cursor-not-allowed rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2"
                >
                  {faceLoading ? (
                    <>
              ...
              {faceError && (
                <div className="mt-3 p-2.5 bg-red-950/30 border border-red-900/50 rounded-lg text-red-300 text-xs">
                  {faceError}
                </div>
              )}

              {entityData && (
                <div className="mt-3 p-3 bg-gradient-to-br from-cyan-950/30 to-blue-950/30 border border-cyan-900/50 rounded-lg">
                  <h3 className="text-xs font-semibold text-cyan-400 mb-2">
                    Identified Entity
                  </h3>
                  <div className="space-y-1.5 text-xs">
                    <div className="flex gap-2">
                      <span className="text-slate-400 w-20">ID:</span>
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-1 text-xs text-cyan-400 hover:text-cyan-300 transition-colors mt-2.5"
                  >
                    View Full Profile
          ...
          {/* Main column: live map, then the registry it drills into */}
          <div className="space-y-4 min-w-0">
            <CampusMap
              heightClass="h-[44vh] min-h-[320px]"
              onExpand={handleRedirect}
              onOpenLocation={(name) =>
            ...
            <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-800/50 rounded-xl overflow-hidden">
              <div className="p-4 border-b border-slate-800/50 flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                <div className="relative sm:ml-auto w-full sm:w-72">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 text-sm bg-slate-800/50 border border-slate-700/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                  />
                </div>
              </div>
              ...
              {loading ? (
                ...
                <div className="overflow-x-auto max-h-[46vh]">
                  <table className="w-full">
                    <thead className="bg-slate-800/30 sticky top-0 backdrop-blur">
                      <tr>
                        {["#", "Entity ID", "Name", "Role", "Face ID", "Department", "Actions"].map(
                          (h) => (
                            <th
                              key={h}
...
function StatTile({
  ...
  return (
    <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-800/50 rounded-xl p-3.5">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-slate-400 text-xs truncate">{label}</p>
          <p className={`font-bold text-white mt-0.5 tabular-nums ${valueClass}`}>{value}</p>
          {hint && <p className="text-[10px] text-slate-500 mt-0.5">{hint}</p>}
        </div>
        <div className={`${tone} p-2 rounded-lg shrink-0`}>{icon}</div>
      </div>
    </div>
  );
}
