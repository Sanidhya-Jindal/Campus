...
# Alerts are derived from a fixed historical dataset, so the computed payload is
# stable; cache it rather than recompute (~60s) on every dashboard load.
ALERTS_CACHE_KEY = "alerts_response_v1"
ALERTS_CACHE_TTL = 60 * 60 * 6  # 6 hours
...
class ProfileViewSet(viewsets.ModelViewSet):
    ...
    queryset = models.Profile.objects.order_by('entity_id')
    serializer_class = serializers.ProfileSerializer
    lookup_field = 'entity_id'

    def get_queryset(self):
        qs = models.Profile.objects.order_by('entity_id')
        # Slicing must not leak into detail routes: get_object() filters the
        # queryset, which raises TypeError on an already-sliced query.
        if self.action == 'list':
            return qs[:100]
        return qs
...
class AlertsListAPIView(APIView):
    ...
    def get(self, request):
        # The dataset is fixed historical data (SIMULATION_NOW is a constant),
        # so alerts never change. Computing them scans ~330k events and makes
        # several LLM calls (~50s); cache the finished payload so only the very
        # first request (per DatabaseCache, i.e. once total) pays that cost.
        cached = cache.get(ALERTS_CACHE_KEY)
        if cached is not None:
            return Response(cached)

        # The computation holds a DB connection for ~50s, longer than the
        # Supabase pooler's idle timeout, so it may be dropped mid-flight. Retry
        # once on a connection error with a fresh connection.
    ...
    def _compute(self, request):
        ...
        # Filter the 22:00-07:00 window in SQL rather than pulling every
        # Library/Main Building event (~450k rows) into memory to keep 10.
        # TIME_ZONE is UTC, so __hour matches the .hour used previously.
        after_hours_events = models.Event.objects.filter(
            location__in=after_hours_locations,
            entity__role__in=after_hours_roles
        ).filter(
            Q(timestamp__hour__gte=22) | Q(timestamp__hour__lt=7)
        ).select_related('entity').values(
            'entity__entity_id', 'entity__name', 'entity__role', 'timestamp', 'location'
        )[:10]
        ...
        try:
            if os.environ.get("GEMINI_API_KEY"):
                recommender = GeminiAlertRecommender()
        ...
        # The long LLM step can outlast the Supabase pooler's idle timeout, so
        # the connection may be dead by the time we write the cache. Drop it and
        # retry once; the cache is best-effort and must never fail the response.
        try:
            cache.set(ALERTS_CACHE_KEY, alerts_data, ALERTS_CACHE_TTL)
        except Exception:
            from django.db import connection
            connection.close()
            try:
                ...
                print(f"Alerts cache write failed (non-fatal): {e}")
...
class OccupancyAllAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Predictions depend only on future_time and the (static) occupancy data.
        # The frontend snaps requests to the same 15-min block, so every client
        # on the dashboard shares one cache entry instead of retraining models.
        forecast_cache_key = f"forecast_all_{future_time_str}"
        ...
        try:
            ...
            # 9. Cache and return the full list of predictions
            cache.set(forecast_cache_key, results, 60 * 20)  # 20 min
